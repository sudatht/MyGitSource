...
<%
	...
	if (ExecuteCRUDSQL(strSQL, Conn) = false) then
		...
		set Conn = nothing	   
		AddErrorMessage("Feil under oppdatering av buntnr på fakturagrunnlag.")
	...
	strSQL = "SELECT DISTINCT 	[F].[FirmaID],[K].[so_number2], [K].[SOCuID], [K].[Firma],  " &_
		"[F].[Kontakt], DeresRef = ([Ko].[Fornavn] + ' ' + [Ko].[Etternavn]), [F].[SOKontakt],  " &_
		"[F].[Fakturanr], [F].[OppdragID],[O].[FaID], [F].[avdeling], vaaresRef = ([M].[Fornavn] + ' ' + [M].[Etternavn]), [O].[AnsmedID], [O].[SoPeID] " &_
	...
	Conn.BeginTrans
	'	Loop for å overføre til ordrehode
	do while NOT rsID.EOF

		'KundeNR = rsID("FirmaID")
		KundeNR = rsID("so_number2")
		HovedbokKtoNr = 0
		Avdeling = rsID("Avdeling")
		faID = 0
		if(not isnull(rsID("FaID"))) then
			faID = rsID("FaID")
		end if
	   ...
	   if (ExecuteCRUDSQL(strSQL, Conn) = false) then
		...
		strSQL = "INSERT INTO [EKSPORT_RUB_ORDRE](" &_
			...
			"[Eksportert_Dato], [Avdeling], [Kontakt], [SOKontakt], [FaktDatoType],[Rabattgruppe], [Buntnr]" &_
			") VALUES(" &_
			rsID("Fakturanr") &_
			",6000, 1, 6, 1, 1," &_
			Ordrenummer & ", " &_
