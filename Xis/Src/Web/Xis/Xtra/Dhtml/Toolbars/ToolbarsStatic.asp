...
function TBLayoutToolbars() {
  ...
  x = <%=request("lLeftPadding")%>; y = <%=request("lTopPadding")%>;
...
function TBLayoutBodyElement() {
  ...
  tbContentElementObject.style.left = <%=request("lLeftPadding")%>; 
  tbContentElementObject.style.posHeight = <%=request("lEditorHeight")%>;//document.body.offsetHeight - tbContentElementBottom - tbContentElementTop;
  tbContentElementObject.style.width = <%=request("lEditorWidth")%>; //document.body.offsetWidth;
...
function document.onreadystatechange() {
  ...
  //Fred.myklebust@electricfarm.no, 12.12.02 -> must allow scrolling in editor document
